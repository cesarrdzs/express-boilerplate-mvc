...
var BIN_PATH = path.resolve(path.dirname(PKG_PATH), require(PKG_PATH).bin.expressbo)
	...
	describe('(no args)', function () {
		...
		it('should create basic app', function (done) {
			runRaw(ctx.dir, [], function (err, code, stdout, stderr) {
				...
				ctx.stderr = stderr
				ctx.stdout = stdout
				assert.equal(ctx.files.length, 27)
				done()
			})
		})
		...
		it('should have basic files', function () {
			...
			assert.notEqual(ctx.files.indexOf('index.js'), -1)
